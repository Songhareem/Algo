...
// Map(hash) 기본적인 사용법
// forEach 사용법
// - Map<any, any>.forEach(callbackfn: (value: any, key: any, map: Map<any, any>) => void, thisArg?: any): void
...
function solution(participant, completion) {
    ...
    for(var key of participant) {
        if(participantMap.has(key)) {
            participantMap.set(key, participantMap.get(key)+1)
        } else {
            participantMap.set(key, 1);
        }
    } 

    for(var key of completion) {
        if(completionMap.has(key)) {
            completionMap.set(key, completionMap.get(key)+1)
        } else {
            completionMap.set(key, 1);
        }
    }
